...
TEST_CASE("ParserItems:mine_info", "[parser_items]")
{
  const auto xml = readFile("resources/Lethos.irt");
  ParserItems parser;
  parser.parse(xml);

  REQUIRE(parser.getZones()[0].minesInfo.saw.name == "Sawmill");
  REQUIRE(parser.getZones()[0].minesInfo.saw.type == "mill");
  REQUIRE(parser.getZones()[0].minesInfo.saw.chance == 1.0);
  REQUIRE(parser.getZones()[0].minesInfo.saw.maxNumber == 1);
  REQUIRE(parser.getZones()[0].minesInfo.saw.value == 0);
}